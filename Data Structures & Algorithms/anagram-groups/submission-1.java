class Solution {
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for(int i=0;i<strs.length;i++){
            ArrayList<Integer> left = new ArrayList<>(Collections.nCopies(26, 0));
            for(int j=0;j<strs[i].length();j++){
                int ascii = strs[i].charAt(j) - 'a';
                left.set(ascii,left.get(ascii) + 1);
            }
            if(ans.containsKey(left)){
                ArrayList<String> putObject = ans.get(left);
                putObject.add(strs[i]);
                ans.put(left,putObject);
            }
            else{
